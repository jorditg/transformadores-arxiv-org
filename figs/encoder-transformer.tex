...
\node (startn) [start, text width=2cm] {Entradas};
%\node (enc) [process, below of=start] {Codificador};
\node (dec) [process, below of=startn, text width=2cm] {Codificadores en serie};
...
\node (out) [stop, below of=clas, text width=2cm] {Salida};

%\node (pro1) [process, below of=in1] {Process 1};
%\node (dec1) [decision, below of=pro1, yshift=-0.5cm] {Decision 1};

\draw [arrow, yshift=1cm] (startn) -- (dec);
%\draw [arrow] (enc) -- (dec);
\draw [arrow] (dec) -- (clas);
\draw [arrow] (clas) -- (out);
%\draw [arrow] (outs) -- (dec);
